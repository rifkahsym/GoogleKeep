...
const CreateChecklistScreen = ({ navigation }) => {
    ...
    const toggleColorPopup = () => setColorPopupVisible(!isColorPopupVisible);
    const toggleActionPopup = () => setActionPopupVisible(!isActionPopupVisible);
        ...
        <SafeAreaView style={[styles.container, darkMode && styles.darkContainer]}>
            ...
            <View style={styles.checkboxContainer}>
                {['Project 1'].map((item, index) => (
                    <TouchableOpacity key={index} onPress={() => toggleCheckbox(index)} style={styles.checkbox}>
                        <Ionicons name={checkedItems[index] ? "checkmark-circle" : "square"} size={24} color={darkMode ? 'white' : 'black'} />
                        <Text style={{ color: darkMode ? 'white' : 'black' }}>{item}</Text>
                    </TouchableOpacity>
                ))}
            </View>
